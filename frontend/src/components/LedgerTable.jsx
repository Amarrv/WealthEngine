        ...
        {(!transactions || transactions.length === 0) ? (
          <div className="p-20 text-center text-zinc-500 flex flex-col items-center gap-3">
            ...
            <div className="space-y-1">
              <p className="text-sm font-medium text-zinc-300">No transactions for this date range.</p>
              <p className="text-xs text-zinc-600">Try selecting a wider range or logging a new entry.</p>
            </div>
            <Button variant="outline" size="sm" onClick={resetAllFilters} className="mt-2 bg-indigo-500/10 border-indigo-500/20 text-indigo-400 hover:bg-indigo-500/20 rounded-xl">
              Show this month's activity
            </Button>
          </div>
          ...
          <div className="p-20 text-center text-zinc-500 flex flex-col items-center gap-3">
            ...
            <Button variant="ghost" size="sm" onClick={() => { setSearchQuery(""); setCategoryFilter("ALL"); }} className="text-indigo-400 hover:bg-indigo-500/10">Clear local filters</Button>
          </div>
        ) : (
          <div className="flex flex-col">
            {groupedData.map((group) => (
              <div key={group.dateKey} className="flex flex-col">
                {/* STICKY DATE HEADER */}
                <div className="sticky top-0 z-10 bg-zinc-900/80 backdrop-blur-md px-5 py-3 border-y border-white/5 flex flex-row items-center justify-between gap-2 shadow-sm">
                  ...
                  <div className="flex items-center justify-end gap-3 text-right">
                    {group.summary.totalIncome > 0 && (
                      <div className="flex items-center gap-1.5 px-2 py-1 bg-emerald-500/5 rounded-lg border border-emerald-500/10">
                        <span className="text-[8px] text-zinc-500 uppercase tracking-tighter">Income</span>
                        <span className="text-xs font-mono font-bold text-emerald-400">+₹{Math.round(group.summary.totalIncome).toLocaleString()}</span>
                      </div>
                    )}
                    <div className="flex items-center gap-1.5 px-2 py-1 bg-rose-500/5 rounded-lg border border-rose-500/10">
                      <span className="text-[9px] text-grey-500 uppercase tracking-tighter">Total</span>
                      <span className="text-xs font-mono font-bold text-rose-500">-₹{Math.round(group.summary.totalExpense).toLocaleString()}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex flex-col divide-y divide-white/5">
                  {group.transactions.map((tx) => {
                    ...
                    return (
                      <div
                        key={tx._id}
                        className={`group p-4 px-5 flex flex-col gap-2 hover:bg-white/[0.03] transition-all duration-200 ${isHighValue ? "bg-indigo-500/[0.03]" : ""}`}
                      >
                          ...
                          <div className="flex items-center gap-3 shrink-0">
                            <div className={`font-mono font-bold tabular-nums whitespace-nowrap transition-all ${isHighValue ? "text-lg md:text-xl" : "text-sm md:text-base"} ${baseColor}`}>
                              {sign}₹{parseFloat(tx.amount).toLocaleString("en-IN", {
                                ...
                                maximumFractionDigits: (parseFloat(tx.amount) % 1 === 0) ? 0 : 2,
                              })}
                            </div>

                            <Popover>
                              <PopoverTrigger asChild>
                                <button className="p-1 hover:bg-white/10 rounded-lg text-zinc-600 hover:text-zinc-300 transition-colors">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {isHighValue && <span className="text-[9px] text-indigo-400/60 uppercase font-bold tracking-widest ml-auto">High Value</span>}
                        </div>
                      </div>
                    );
                  })}
